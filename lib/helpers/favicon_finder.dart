...
class FaviconFinder {
  static Future<List<Favicon>> getAll(
    ...
    for (var iconUrl in iconUrls) {
      // Exclude URLs with MIME type 'image/svg+xml' or ending with '.svg'
      var contentType = await _getImageContentType(iconUrl);
  ...
  static Future<String?> _getImageContentType(String imageUrl) async {
    try {
